...
# -*- coding: utf-8 -*-
"""
BCUR crawler - Step 1 (streaming CSV + resume) - Minimal Equivalent
- Merge MySQL execute helpers into one: _run
- Remove redundant skip_offset reset (only determined by start_idx once)
- Extract get_rows() to avoid duplicated DOM code
"""
...
class TaskCfg:
    ...
    db_table: str
    db_columns_en: list[str]
    csv_to_db_map: dict[str, str]
...
TASKS: dict[str, TaskCfg] = {
    "bcur": TaskCfg(
        ...
        xpaths_per_col=[
            ["./td[2]/div/div[2]/div[1]/div/div/span"],  # name
            ["./td[2]/div/div[2]/p"],                     # tags
            ["./td[3]"],                                  # province
            ["./td[4]"],                                  # category
            ["./td[5]"],                                  # score
        ],
...
class MySQLWriter:
    ...
    def insert_dataframe(self, df: pd.DataFrame, cfg: TaskCfg) -> int:
        ...
        for _, r in df.iterrows():
            rank_num = int(str(r.get("排名", "0")).strip() or "0")
            values: dict[str, Any] = {
                "rank": rank_num,
                "name": str(r.get("学校名称", "")).strip(),
            }
            for csv_cn, db_en in cfg.csv_to_db_map.items():
                if db_en in ("rank", "name"):
                    continue
...
# ======= main flow =======
def run(enable_db: bool, out_dir: str) -> None:
        ...
        if enable_db:
            with MySQLWriter(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME) as writer:
                ...
                for chunk in pd.read_csv(fpath, chunksize=DB_CHUNK_SIZE):
                    ...
                    writer._conn.commit()
                    inserted += affected
                    logger.info("DB progress: affected=%d, cumulative=%d", affected, inserted)
                logger.info("DB insert finished: total_rows=%d", inserted)
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="BCUR crawler (Step 1 - minimal equivalent)")
    p.add_argument("--no-db", action="store_true", help="Disable DB writes")
    p.add_argument("--out-dir", type=str, default=".", help="CSV output directory")
    return p.parse_args(argv)

def main() -> None:
    args = parse_args()
    enable_db = not args.no_db
    run(enable_db=enable_db, out_dir=args.out_dir)
